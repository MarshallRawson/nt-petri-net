...
impl eframe::App for PlotMuxUi {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.show_graph {
                ui.image(self.graph_texture.as_ref().unwrap(), self.graph_texture.as_ref().unwrap().size_vec2());
            } else {
                ui.horizontal(|ui| {
                    ui.label("Source: ");
                                    ...
                                    .show(ui, |plot_ui| {
                                        for (name, (color, vec)) in &self.sources[source_idx].series_2d {
                                            let plot_vec = {
                                                ...
                                                else if let Some(start) = vec.iter().position(
                                                    |&v| v.x > vec.last().unwrap().x - self.series_2d_history
                                                ) {
                                                    vec[start..].to_vec()
                                                }
                                                else {
