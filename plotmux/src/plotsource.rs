use crate::plotmux::{PlotableData, Color, color};
    ...
    pub fn new_data(&mut self, d: PlotableData) {
        match d {
            ...
            PlotableData::Series2d(series_2d) => {
                match self.series_2d.get_mut(&series_2d.series) {
                    Some(points) => {
                        points.1.push(Value::new(series_2d.x, series_2d.y));
                    },
                    None => {
                        let c = color(&series_2d.series);
                        self.series_2d.insert(series_2d.series, (c, vec![Value::new(series_2d.x, series_2d.y)]));
                    },
                }
            },
        }
