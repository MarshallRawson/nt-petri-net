use image::{RgbImage, GrayImage, imageops::{FilterType, resize}};
        ...
        for bbox in &bboxes {
            ...
            for i in 0 .. mouth.width() {
                for j in 0 .. mouth.height() {
                    let i2 = (i + mouth_center.0 as u32 - mouth.width() / 2).clamp(0, faces.width()-1);
                    let j2 = (j + mouth_center.1 as u32 - mouth.height() / 2).clamp(0, faces.height()-1);
            ...
            if self.hist.len() >= 30 {
                ...
                self.p.plot_series_2d("", "brightness", (t - self.t0).as_secs_f64(), bright);
                let w =  mouth.width();
                let h =  mouth.height();
                self.p.plot_image("",
                    resize(&DynamicImage::ImageLuma8(mouth).into_rgb8(), w*15, h*15, FilterType::Triangle),
                    ImageCompression::Lossless
                );
                moving.push(bright);
            }
        }
        Output::Moving(Moving{moving: (t, moving), done: ()})
    }
}
